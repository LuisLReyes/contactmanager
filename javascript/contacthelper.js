    ...
    $.ajax({
        ...
        success : function(result){
            ...
            result.data.forEach(contact =>  {
                var row = '<tr class="table-info" id = '+contact.id+'><td id="first_name'+contact.id+'">'+JSON.stringify(contact.first_name).replace(/['"]+/g, '')+'</td>' +
                    ...
                    '<td id="buttons'+contact.id+'" style="text-align:center"> <button class="btn btn-success oi oi-pencil" style="width:40%" onclick="editContact('+contact.id+')"></button><button class="btn btn-danger oi oi-trash" style="width:40%" onclick="deleteContact('+contact.id+')"></button></td>' +
